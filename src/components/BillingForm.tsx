...
const BillingForm = ({subscriptionPlan}: BillingFormProps) => {
    ...
    const {mutate: createStripeSession, isLoading} = trpc.createStripeSession.useMutation({
        onSuccess: ({url}) => {
            ...
            if(!url)    toast({title: "There was a problem... :(", description: "PLease try again in a moment", variant: "destructive"})
        }
    })
    ...
    return <MaxWidthWrapper className="max-w-5xl">
        <form className="mt-12" onSubmit={(e) => {
            ...
            <Card>
                ...
                <CardFooter className="flex flex-col items-start pace-y-2 md:flex-row md:justify-between md:space-x-0">
                    <Button type="submit">
                        {isLoading? (
